...
X_train = df.loc[train_idx, config.FEATURES]
y_train = df.loc[train_idx, config.TARGET]

X_val = df.loc[val_idx, config.FEATURES]
y_val = df.loc[val_idx, config.TARGET]
...
df["Predicted Invoice Price"] = df["Invoice Price"].round(2)
